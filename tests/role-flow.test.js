...
async function __acceptanceFlow(){
  ...
  renderAuthState();
  if (document.getElementById("tab-budget")?.hidden) throw new Error("Owner budget nav hidden");
  ...
  if (!document.getElementById("tab-budget")?.hidden) throw new Error("Admin budget nav rendered");
  if (!document.getElementById("tab-buildings")?.hidden) throw new Error("Admin buildings nav rendered");
  ...
  if (!document.getElementById("openAddNewBtn")?.hidden) throw new Error("Submitter command Add New rendered");
  renderSettings();
  if (!document.getElementById("diagnosticPanel")?.hidden) throw new Error("Submitter diagnostic panel rendered");
  showView("reports");
  if (activeViewId !== "fieldPortal") throw new Error("Submitter direct admin route did not redirect home");
